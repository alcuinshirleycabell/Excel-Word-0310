...
def find_best_match(word_text, excel_cols, threshold=0.6):
...
def build_mapping(row, excel_cols=None):
    mapping = {}
...
def replace_smart(paragraph, mapping, excel_cols=None):
...
def replace_in_doc(doc, mapping, excel_cols=None):
    total = 0
...
class App:
    def __init__(self, root):
        ...
        ttk.Entry(frm, textvariable=self.filename_col, width=24).grid(row=3, column=1, sticky=tk.W, padx=6, pady=(10, 0))

        ttk.Label(frm, text="自定义文件名模板（可选）：").grid(row=4, column=0, sticky=tk.W, pady=(6, 0))
        ttk.Entry(frm, textvariable=self.filename_pattern, width=74).grid(row=4, column=1, padx=6, pady=(6, 0), sticky=tk.W)
        ttk.Label(frm, text="示例：证书_{姓名}_{编号}；可用 {序号} 表示行号", foreground="#666666").grid(row=5, column=1, sticky=tk.W)
    ...
    def render_filename(self, row, idx, name_col, pattern):
        if pattern:
            rendered = pattern
            for col, val in row.items():
                col = col.strip()
    ...
    def worker(self):
        try:
            ...
            if name_col and name_col not in df.columns:
                self._set_status(f"文件名列不存在，改用默认命名")
                name_col = ""
            ...
            for idx, row in df.iterrows():
                ...
                doc.save(out_path)
                self._set_status(f"已生成 {idx + 1}/{total}：{os.path.basename(out_path)}")
...
def run_cli():
    ...
    parser.add_argument("--filename-col", default="")
    ...
    for idx, row in df.iterrows():
        ...
        doc.save(out_path)
        print(f"[{idx + 1}/{total}] {os.path.basename(out_path)}")
